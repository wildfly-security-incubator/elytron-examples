...
public class SecuredServlet extends HttpServlet {
    ...
    private void displayScopeClaims(String scope, List<String> message, OidcSecurityContext oidcSecurityContext) {
        ...
        if (scope.contains("profile")){
            message.add("Using the \"profile\" scope, we got User's name: " + oidcSecurityContext.getToken().getClaimValueAsString("given_name") + " "
                    + oidcSecurityContext.getToken().getClaimValueAsString("family_name"));
        }
        if (scope.contains("email")){
            message.add("Using the \"email\" scope, we got email verified: " + oidcSecurityContext.getToken().getClaimValueAsString("email_verified") + "\n");
        }
        if (scope.contains("roles")){
            message.add("Using the \"roles\" scope, we got user's roles: " + oidcSecurityContext.getToken().getRealmAccessClaim().getRoles().toString() + "\n");
        }
        if (scope.contains("acr")){ //authentication context class reference scope
            message.add("Using the \"acr\" scope, we got authentication context class reference: " + oidcSecurityContext.getToken().getClaimValueAsString("acr") + "\n");
        }
        if (scope.contains("microprofile-jwt")){
            message.add("Using the \"microprofile-jwt\" scope, we got the user's groups: " + oidcSecurityContext.getToken().getClaimValueAsString("groups") + "\n");
        }
        if (scope.contains("web-origins")){
            message.add("Using the \"web-origins\" scope, we got the user's allowed-origins: " + oidcSecurityContext.getToken().getClaimValueAsString("allowed-origins") + "\n");
        }
    }
}
